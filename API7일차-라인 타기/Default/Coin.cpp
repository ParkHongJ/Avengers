...
void CCoin::Render(HDC hDC)
{
	...
	Ellipse(hDC, m_tRect.left + iScrollX, m_tRect.top, m_tRect.right + iScrollX - 5, m_tRect.bottom - 1);
	...
	wsprintf(lpOut, TEXT("$"));
	TextOut(hDC, m_tRect.left + iScrollX + 10, m_tRect.top + 6, lpOut, lstrlen(lpOut));
}
